...
struct Hasher {
    ...
    // Zero copy memory version
    Chunk* zcopy_factory[FACTORY_HT];
    u32 zcopy_sizes[FACTORY_HT];
    ...
    ~Hasher() {
        for(int i=0; i<FACTORY_HT; i++) {
            cudaHostUnregister(zcopy_factory[i]);
            // delete[] zcopy_factory[i];
            cudaFreeHost(zcopy_factory[i]);
        }
    }
...
void Hasher::init() {
    for(int i=0; i<FACTORY_HT; i++) {
        // zcopy_factory[i] = new Chunk[SNICKER];
        cudaHostAlloc(&(zcopy_factory[i]), SNICKER*sizeof(Chunk), cudaHostAllocMapped);
        cudaHostRegister(zcopy_factory[i], SNICKER*sizeof(Chunk), cudaHostRegisterMapped);
        zcopy_sizes[i] = 0;
    }
}
...
void Hasher::propagate() {
    ...
    while(zcopy_sizes[level] == SNICKER) {
        // cout << "Hashing many\n";
        Chunk subtree = hash_many(zcopy_factory[level], 0, SNICKER);
        zcopy_sizes[level] = 0;
        ++level;
        zcopy_factory[level][zcopy_sizes[level]++] = subtree;
        // factory[level].push_back(subtree);
    }
} 

void Hasher::update(char *input, int size) {
    zcopy_factory[0][zcopy_sizes[0]++] = Chunk(input, size, flags, key, ctr);
    ++ctr;
    if(zcopy_sizes[0] == SNICKER)
        propagate();
}
...
void Hasher::finalize(vector<u8> &out_slice) {
    ...
    for(int i=0; i<FACTORY_HT; i++) {
        ...
        u32 n = zcopy_sizes[i], divider=SNICKER;
        if(!n)
            continue;
        int start = 0;
        while(divider) {
            if(n&divider) {
                Chunk subtree = hash_many(zcopy_factory[i], start, start+divider);
        ...
        if(i<FACTORY_HT-1)
            zcopy_factory[i+1][zcopy_sizes[i+1]++] = subtrees[0];
        else
            root = subtrees[0];
    }
