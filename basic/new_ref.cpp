    ...
    void update(vector<u8> &input)
    {
        while (!input.empty())
        {
            if (u32(block_len) == BLOCK_LEN)
            {
                ...
                u32* convert_this=first_8_words(para);
                for(int k=0; k<8; k++)
                {
                    chaining_value[k]=para[k];
                }
                //chaining_value end
                blocks_compressed += 1;
